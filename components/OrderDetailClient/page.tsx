...
type FullWorkOrder = Awaited<ReturnType<typeof import('@/lib/data').getWorkOrderDetail>>;
...
export default function OrderDetailClient({ order: initialOrder }: OrderDetailClientProps) {
    ...
    const canEditGeneral = isManager || (currentUserId === initialOrder.createdById);
    ...
    const handleUpdate = async (field: keyof FullWorkOrder, newValue: string | null) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {isManager ? (
                    ...
                    <div>
                        ...
                        <p className="text-lg font-medium">{order.assignedTo?.name || 'Unassigned'}</p>
                    </div>
                )}
                
                <div>
                    <span className="label">Created By</span>
                    <p className="text-lg font-medium">{order.createdBy.name} ({order.createdBy.email})</p>
                </div>

                <div>
                    <span className="label">Created At</span>
                    <p className="text-lg font-medium">{new Date(order.createdAt).toLocaleString()}</p>
                </div>
            </div>
...
function ManagerAssigneeField({ label, order, allUsers, onSave }: AssigneeFieldProps) {
    const [currentId, setCurrentId] = useState(order.assignedToId || 'unassigned');
    const assignedName = allUsers.find(u => u.id === order.assignedToId)?.name || 'Unassigned';
